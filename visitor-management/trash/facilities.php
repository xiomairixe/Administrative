...
<body>
  ...
  <div class="main-content">
      ...
      <div class="content-body">
        ...
        <div class="row g-4">
          ...
          <div class="col-md-4 facility-card">
            ...
            <?php } else { ?>
              <button class="btn btn-sm" style="background:#ab83fcff;" data-bs-toggle="modal" 
    ...
    <?php
    $reservation_id = null;
    $res = $conn->query("SELECT request_id FROM reservation_requests WHERE facility_id = $facility_id AND status = 'Approved' ORDER BY requested_at DESC LIMIT 1");
    if ($res && $res->num_rows > 0) {
        $reservation_row = $res->fetch_assoc();
        $reservation_id = $reservation_row['request_id'];
    }
    ?>
    <!-- Head Visitor Modal -->
        ...
        <form action="action/register_head_visitor.php" method="post">
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              <div class="mb-3">
                <label>Pass Code for Request (12 digits)</label>
                <input type="text" name="pass_code" class="form-control" maxlength="12" pattern="\d{12}" required placeholder="Enter 12-digit code">
                <small class="text-muted">Ask the requestor for the code generated upon approval.</small>
              </div>
              <div class="mb-3">
...
if (isset($_POST['register_head'])) {
    ...
    $check->store_result();
    if ($check->num_rows == 0) {
        echo "<script>alert('Reservation does not exist.'); window.location.href='facility.php';</script>";
        exit();
    }
    $check->close();

    // Check if pass code matches
    $codeCheck = $conn->prepare("SELECT code FROM code WHERE request_id = ? AND code = ?");
    $codeCheck->bind_param("is", $reservationID, $passCode);
    $codeCheck->execute();
    $codeCheck->store_result();
    if ($codeCheck->num_rows == 0) {
        echo "<script>alert('Invalid pass code.'); window.location.href='facility.php';</script>";
        exit();
    }
    $codeCheck->close();

    // Insert head visitor
    $insertVisitor = $conn->prepare("INSERT INTO visitors (full_name, email, contact_number, reservation_id) VALUES (?, ?, ?, ?)");
    $insertVisitor->bind_param("sssi", $fullName, $email, $contact, $reservationID);

    if ($insertVisitor->execute()) {
        echo "<script>alert('Head visitor registered successfully.'); window.location.href='facility.php';</script>";
    } else {
        echo "<script>alert('Error registering head visitor.'); window.location.href='facility.php';</script>";
    }
