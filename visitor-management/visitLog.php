<?php
  ...
  // JOIN visit_log for check_in and check_out times
  $sql = "SELECT v.*, vl.check_in, vl.check_out
          FROM visitors v
          LEFT JOIN visit_log vl ON v.visitor_id = vl.visitor_id";
  if ($where) {
    ...
    <div class="topbar mb-4">
      <div class="d-flex align-items-center gap-3">
        <div class=" justify-content-between align-items-center mb-3 flex-wrap gap-2">
          <div class="dashboard-title">Visitor Log</div>
          <div class="breadcrumbs">View and search visitor histor</div>
        </div>  
      ...
      <table class="table table-striped align-middle">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              <span class="badge bg-success"><?= !empty($row['check_in']) ? date('M d, Y H:i', strtotime($row['check_in'])) : '-' ?></span>
            </td>
            <td>
              <span class="badge bg-danger"><?= !empty($row['check_out']) ? date('M d, Y H:i', strtotime($row['check_out'])) : '-' ?></span>
            </td>
          ...
          <div class="modal fade" id="detailsModal<?= $row['visitor_id'] ?>" tabindex="-1" aria-labelledby="detailsModalLabel<?= $row['id'] ?>" aria-hidden="true">
                  ...
                  <div class="row mb-3">
                    ...
                    <div class="col-md-9">
                      <dl class="row mb-0">
                        ...
                        <dd class="col-sm-8"><?= !empty($row['check_in']) ? date('M d, Y H:i', strtotime($row['check_in'])) : '-' ?></dd>
                        <dt class="col-sm-4">Check-Out:</dt>
                        <dd class="col-sm-8"><?= !empty($row['check_out']) ? date('M d, Y H:i', strtotime($row['check_out'])) : '-' ?></dd>
                        ...
                        <d class="col-sm-8"><?= !empty($row['consent_privacy']) ? 'Yes' : 'No' ?></dd>
          ...
          <?php if ($visitor->num_rows === 0): ?>
            <tr><td colspan="11" class="text-center text-muted">No visitor records found.</td></tr>
          <?php endif; ?>
        </tbody>
      </table>
    </div>
