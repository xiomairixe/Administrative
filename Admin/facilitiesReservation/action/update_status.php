...
if (isset($_GET['request_id']) && isset($_GET['status']) && $_GET['status'] == 'Approved') {
    $reservation_id = $_GET['request_id'];

    // Update reservation to Approved
    $updateReservation = "UPDATE reservation_requests SET status = 'Approved' WHERE request_id = '$reservation_id'";
    if ($conn->query($updateReservation)) {
        ...
        if ($result && $result->num_rows > 0) {
            ...
            $updateFacilityStatus = "UPDATE facilities SET status = 'Reserved' WHERE facility_name = '$facilityName'";
