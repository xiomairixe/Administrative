...
<div class="container-fluid p-0">
  <div class="row g-0">
        ...
        <div class="">
          ...
          <div class="mb-4">
            ...
            <nav class="nav flex-column">
              ...
              <a class="nav-link active" href="#"><ion-icon name="checkmark-done-outline"></ion-icon>Review & Approve</a>
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
          <h4 class="mb-3"><?= htmlspecialchars($document['title']) ?></h4>
          <p class="text-muted">Review and Provide Feedback on this Document.</p>
        </div>
      </div>
      ...
      <div class="d-flex justify-content-between align-items-start mb-4">
        ...
        <div class="d-flex gap-2 top-actions">
          <button id="rejectBtn" class="btn btn-outline-secondary">Reject</button>
          <button id="approveBtn" class="btn btn-warning text-white fw-semibold">Approve</button>
        </div>
      </div>

      <ul class="nav nav-tabs mb-3" id="documentTabs" role="tablist">
        <li class="nav-item" role="presentation">
          <button class="nav-link active" id="doc-tab" data-bs-toggle="tab" data-bs-target="#doc" type="button" role="tab">Document</button>
        </li>
        <li class="nav-item" role="presentation">
      ...
      <div class="d-flex justify-content-between mb-3">
        <small class="text-muted">Version <?= htmlspecialchars($document['version'] ?? '1.0') ?> | Uploaded by <?= htmlspecialchars($document['uploaded_by'] ?? 'Unknown') ?> on <?= htmlspecialchars($document['upload_date'] ?? '') ?></small>
        <a href="download_document.php?id=<?= $docId ?>" class="btn btn-outline-secondary btn-sm">
          <ion-icon name="download-outline"></ion-icon> Download
        </a>
      </div>

      <div class="tab-content mb-4">
        <!-- Document content, you can enhance this to show actual document text or preview -->
        <div class="tab-pane fade show active" id="doc" role="tabpanel">
          <div>
            <?= nl2br(htmlspecialchars($document['content'] ?? 'Document content not available.')) ?>
          </div>
        </div>
        ...
        <div class="tab-pane fade" id="comments" role="tabpanel">
          <div id="commentsList">
            <?php foreach ($comments as $comment): ?>
              <div class="mb-3 p-2 border rounded bg-white">
                <strong><?= htmlspecialchars($comment['commenter_name']) ?></strong><br>
          ...
          <div class="mt-4">
            <textarea id="newComment" class="form-control" rows="3" placeholder="Write a comment..."></textarea>
          ...
          <ul class="list-group">
            <?php foreach ($history as $event): ?>
              <li class="list-group-item">
                <strong><?= htmlspecialchars($event['name'] ?? 'System') ?></strong> <?= htmlspecialchars($event['action']) ?><br>
                <small class="text-muted"><?= htmlspecialchars($event['action_at']) ?></small>
              </li>
            <?php endforeach; ?>
          </ul>
        </div>
      ...
      <div class="mt-4 p-3 border rounded bg-light">
        ...
        <div class="row">
          <div class="col-md-3">
            ...
            <p id="statusText" class="mb-0"><?= htmlspecialchars(ucfirst($document['status'])) ?></p>
          </div>
          <div class="col-md-3">
            <strong>Type:</strong>
            <p class="mb-0"><?= htmlspecialchars(strtoupper(pathinfo($document['file_name'], PATHINFO_EXTENSION) ?: 'PDF')) ?></p>
          </div>
          <div class="col-md-3">
            <strong>Workflow:</strong>
            <p class="mb-0"><?= htmlspecialchars($document['workflow'] ?? 'N/A') ?></p>
          </div>
          <div class="col-md-3">
            <strong>Due Date:</strong>
            <p class="mb-0"><?= htmlspecialchars($document['due_date'] ?? 'N/A') ?></p>
          </div>
        </div>
      </div>
    </div>
